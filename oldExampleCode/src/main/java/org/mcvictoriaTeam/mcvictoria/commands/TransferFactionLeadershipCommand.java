...
public class TransferFactionLeadershipCommand implements CommandExecutor {
    public TransferFactionLeadershipCommand(Main plugin) {
        plugin.getCommand(Factions.FACTION_COMMAND_TRANSFER_LEADERSHIP).setExecutor(this);
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase(Factions.FACTION_COMMAND_PROMOTE_MEMBER)) {
            ...
            if (!factionRank.equals(Factions.FACTION_LEADER_RANK)) {
                player.sendMessage("Error: You may only transfer leadership if you are the faction leader.");
                return true;
            }
            ...
            if (targetPlayerRank.equals(Factions.FACTION_OFFICIAL_RANK)) {
                Factions.removeOfficialFromFaction(targetPlayer.getUniqueId().toString(), factionName);
            }

            Factions.setFactionLeader(targetPlayer.getUniqueId().toString(), factionName);
        }
