...
public class DemoteToFactionMemberCommand implements CommandExecutor {
    public DemoteToFactionMemberCommand(Main plugin) {
        plugin.getCommand(Factions.FACTION_COMMAND_DEMOTE_OFFICIAL).setExecutor(this);
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase(Factions.FACTION_COMMAND_PROMOTE_MEMBER)) {
            ...
            if (!factionRank.equals(Factions.FACTION_LEADER_RANK)) {
                player.sendMessage("Error: You may only demote members if you are the faction leader.");
                return true;
            }
            ...
            if (!targetPlayerRank.equals(Factions.FACTION_OFFICIAL_RANK)) {
                player.sendMessage("Error: Only players of the 'official' rank can be demoted.");
                return true;
            }
